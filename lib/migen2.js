...
(
	function()
	{
		...
		*/
		...
		var btnCDZoomIn=$('button#cdzoomin').button({});
		var btnCDZoomOut=$('button#cdzoomout').button({});
		var btnGAZoomIn=$('button#gazoomin').button({});
		var btnGAZoomOut=$('button#gazoomout').button({});
		...
		*/
		...
		function displayGoals()
		{
			...
			var exists=$("table#goals").length!=0;
			...
			if(exists==true)
			{
				...
				$('tbody#usergoalslist').append(rowstext);
			}
			//otherwise create it
			else
			{
		...
		(
			function()
			{
				if(browserDetect.browser=="Firefox")
				{
					var zoom=$('.goalsTable').css("-moz-transform");
					var array=zoom.split(",");
					var zoom_value=parseFloat(array[3])+0.1;
					var new_value="matrix("+zoom_value+",0,0,"+zoom_value+",0,0)";
					$('.goalsTable').css("-moz-transform",new_value);
				}
				else
				{
					var zoom=$('.goalsTable').css("zoom");
					zoom=parseFloat(zoom)+0.1;
					$('.goalsTable').css("zoom",zoom);
				}
			}
		);
		...
		(
			function()
			{
				$("div#progressbar").progressbar("option","value",0);
				displayMessageInBox(getFormattedTime()+"fetching data...");
				refreshData();
				$("div#progressbar").progressbar("option","value",40);
				displayMessageInBox(getFormattedTime()+"local data updated...");
				//testData();
				displayUsers();
				paintUsers();
				$("div#progressbar").progressbar("option","value",70);
				displayMessageInBox(getFormattedTime()+"class dynamics view updated...");
				displayGoals();
				displayScore();
				$("div#progressbar").progressbar("option","value",100);
